...
if IS_WINDOWS:
    ...
    def _win_move_absolute(x, y):
        ...
        _last_inj[0] = nx   # remember for _win_click
        _last_inj[1] = ny
        inp = _make_mi(nx, ny, _MEF_MABS)
    ...
    def _win_click(button, pressed):
        ...
        arr = (INPUT * 2)()
        arr[0] = _make_mi(_last_inj[0], _last_inj[1], _MEF_MABS)
...
def _server_worker(log_queue, stop_event, paused_event=None,
    ...
    def check_edge(x, y):
        """Return True if cursor is at a triggerable edge."""
        if not IS_WINDOWS:
            return False
        sw, sh = _win_screen_size()
        return x <= EDGE_PX or x >= sw - EDGE_PX - 1
    ...
    def on_move(x, y):
        if mouse_locked.is_set():
            ...
            if IS_WINDOWS:
                sw, sh = _win_screen_size()
                locked_cursor[0] = max(0.0, min(float(sw - 1), locked_cursor[0] + dx))
                locked_cursor[1] = max(0.0, min(float(sh - 1), locked_cursor[1] + dy))
            else:
                locked_cursor[0] += dx
                locked_cursor[1] += dy
...
def _client_worker(server_ip, log_queue, stop_event, paused_event=None,
    ...
    if IS_WINDOWS:
        ...
        def do_click(b, pressed, x=None, y=None):
            ...
            # Use them to refresh _last_inj *before* the atomic [MOVE+BUTTON]
            # SendInput batch in _win_click fires.
            #
            # Without this, if the mouse was stationary (no preceding mm),
            # TCP coalesced mm+mc into one recv() chunk and the event loop
            # processed mc first, or pynput fired on_click before on_move,
            # _last_inj holds a stale coordinate → the MOVE in the SendInput
            # batch lands at the wrong point → Windows hit-tests the button
            # at the wrong position → drag either targets the wrong window or
            # creates a stuck phantom drag capture.
            if x is not None and y is not None:
                csw, csh = _win_screen_size()
                ssw = _server_res[0] or csw
                ssh = _server_res[1] or csh
                _last_inj[0] = max(0, min(65535, int(x / ssw * 65535)))
                _last_inj[1] = max(0, min(65535, int(y / ssh * 65535)))
...
def launch_gui():
    ...
    _tab_btns = []
    for _i, _name in enumerate(["  ⌨  CONNECT", "  📋  CLIPBOARD"]):
