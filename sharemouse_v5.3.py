...
# ── UAC auto-elevation REMOVED ───────────────────────────────────────────────
# The Interception kernel driver operates below the UIPI / secure-desktop
# boundary, so elevated privileges are no longer required and the UAC
# re-launch block has been removed entirely.
# The _is_admin() helper is kept only for the firewall-rule helper.
if IS_WINDOWS:
...
# ══════════════════════════════════════════════════════════════════════════════
# WINDOWS LOW-LEVEL INPUT  (Interception kernel driver)
# ══════════════════════════════════════════════════════════════════════════════
# WHY Interception instead of SendInput:
#
#   SendInput() is completely blocked when Windows switches to the Secure
#   Desktop for UAC prompts.  No privilege level — not even SYSTEM — can
#   inject input through SendInput on the Secure Desktop.
#
#   The Interception driver is a kernel-mode filter driver that sits BELOW
#   the Secure Desktop isolation layer.  Input injected through Interception
#   reaches the foreground window regardless of UAC prompts, elevated apps,
#   Task Manager, or any integrity level.  This is exactly how Unified Remote
#   and similar professional tools work.
#
#   Prerequisites (one-time per machine):
#     1. Install driver:  install-interception.exe /install  then REBOOT
...
if IS_WINDOWS:
    ...
    # ── Interception driver initialisation ────────────────────────────────────
    # We attempt to load interception-python.  If it is not installed or the
    # driver is not loaded, we fall back to SendInput transparently so the
    # script still works on machines without the driver.

    _icp        = None   # interception module (None = not available)
    _icp_ctx    = None   # interception context handle
    _icp_mouse  = None   # device index for mouse injection
    _icp_kbd    = None   # device index for keyboard injection
    _USE_DRIVER = False  # set True once driver is confirmed working

    def _init_interception():
        """Try to initialise the Interception driver.  Sets _USE_DRIVER = True
        on success.  Silently falls back to SendInput on any failure."""
        global _icp, _icp_ctx, _icp_mouse, _icp_kbd, _USE_DRIVER
        try:
            import interception as _interception_mod
            _icp = _interception_mod
            ctx = _icp.interception_create_context()
            if not ctx:
                return
            _icp_ctx = ctx

            # Find first mouse and keyboard device available for sending
            for i in range(1, 11):
                if _icp.interception_is_mouse(i) and _icp_mouse is None:
                    _icp_mouse = i
                if _icp.interception_is_keyboard(i) and _icp_kbd is None:
                    _icp_kbd = i

            # If no hardware device found, use device slot 11 (virtual send)
            if _icp_mouse is None:
                _icp_mouse = 11   # interception send-only slot
            if _icp_kbd is None:
                _icp_kbd = 1      # interception send-only slot

            _USE_DRIVER = True
            logging.warning("Interception driver active — UAC-proof input injection enabled.")
        except Exception as _e:
            logging.warning(f"Interception driver not available ({_e}), falling back to SendInput.")
            _USE_DRIVER = False

    _init_interception()

    # ── Interception mouse stroke flags ───────────────────────────────────────
    # These mirror the InterceptionMouseFlag enum in the Interception C header.
    _IMOF_MOVE_ABSOLUTE   = 0x0002
    _IMOF_VIRTUAL_DESKTOP = 0x0004   # coords span full virtual desktop

    # ── Interception keyboard stroke flags ────────────────────────────────────
    _IKOF_KEY_DOWN = 0x00
    _IKOF_KEY_UP   = 0x01
    _IKOF_KEY_E0   = 0x02   # extended scan code prefix

    # ── SendInput fallback structures (used when driver not available) ─────────
    ...
    class INPUT(ctypes.Structure):
        _anonymous_ = ("_data",)
        _fields_    = [("type", ctypes.c_ulong), ("_data", _INPUTData)]
    ...
    _MEF_MOVE  = 0x0001
    _MEF_ABS   = 0x8000
    _MEF_MABS  = _MEF_MOVE | _MEF_ABS
    _MEF_WHEEL = 0x0800
    _last_inj  = [32767, 32767]

    def _make_mi(dx, dy, flags, data=0):
        inp = INPUT(); inp.type = 0
        inp.mi.dx = dx; inp.mi.dy = dy
        inp.mi.mouseData = data; inp.mi.dwFlags = flags
        inp.mi.time = 0; inp.mi.dwExtraInfo = 0
        return inp

    # ── coordinate normalisation (shared by both backends) ────────────────────

    def _win_screen_size():
        return (_user32.GetSystemMetrics(0), _user32.GetSystemMetrics(1))

    def _normalise_coords(x, y):
        """Convert server pixel coords → normalised (nx, ny) in [0..65535]
        accounting for virtual desktop layout and monitor offset."""
        vdw = _user32.GetSystemMetrics(78)
        vdh = _user32.GetSystemMetrics(79)
        vdx = _user32.GetSystemMetrics(76)
        vdy = _user32.GetSystemMetrics(77)
        ...
        if vdw <= 0 or vdh <= 0:
            ...
            ox, oy = _monitor_offset[0], _monitor_offset[1]
            px = ox + (x / ssw * mw)
            py = oy + (y / ssh * mh)
    ...
    def _win_move_absolute(x, y):
        nx, ny = _normalise_coords(x, y)
        _last_inj[0] = nx
        _last_inj[1] = ny

        if _USE_DRIVER and _icp_ctx is not None:
            # Interception MouseStroke: absolute move across virtual desktop
            stroke = _icp.InterceptionMouseStroke()
            stroke.state    = 0
            stroke.flags    = _IMOF_MOVE_ABSOLUTE | _IMOF_VIRTUAL_DESKTOP
            stroke.rolling  = 0
            stroke.x        = nx
            stroke.y        = ny
            stroke.information = 0
            _icp.interception_send(_icp_ctx, _icp_mouse, stroke, 1)
        else:
            # SendInput fallback (blocked on Secure Desktop)
            inp = _make_mi(nx, ny, _MEF_MABS | 0x4000)
            _user32.SendInput(1, ctypes.byref(inp), _INPUT_SZ)

    # ── mouse click ───────────────────────────────────────────────────────────

    # Interception mouse button state flags (InterceptionMouseState enum)
    _IMS_BTN = {
        ("left",   True):  0x0001,   # INTERCEPTION_MOUSE_LEFT_BUTTON_DOWN
        ("left",   False): 0x0002,   # INTERCEPTION_MOUSE_LEFT_BUTTON_UP
        ("right",  True):  0x0004,   # INTERCEPTION_MOUSE_RIGHT_BUTTON_DOWN
        ("right",  False): 0x0008,   # INTERCEPTION_MOUSE_RIGHT_BUTTON_UP
        ("middle", True):  0x0010,   # INTERCEPTION_MOUSE_MIDDLE_BUTTON_DOWN
        ("middle", False): 0x0020,   # INTERCEPTION_MOUSE_MIDDLE_BUTTON_UP
    }

    # SendInput fallback button flags
    _BTN_FLAGS = {
        ("left",   True):  0x0002,
        ("left",   False): 0x0004,
        ("right",  True):  0x0008,
    ...
    def _win_click(button, pressed):
        if _USE_DRIVER and _icp_ctx is not None:
            state = _IMS_BTN.get((button, pressed))
            if not state:
                return
            # THE DRAG FIX: send MOVE + BUTTON as one stroke by setting both
            # state (button) and the absolute position (x, y) in the same
            # InterceptionMouseStroke so the kernel sees them atomically.
            stroke = _icp.InterceptionMouseStroke()
            stroke.state       = state
            stroke.flags       = _IMOF_MOVE_ABSOLUTE | _IMOF_VIRTUAL_DESKTOP
            stroke.rolling     = 0
            stroke.x           = _last_inj[0]
            stroke.y           = _last_inj[1]
            stroke.information = 0
            _icp.interception_send(_icp_ctx, _icp_mouse, stroke, 1)
        else:
            # SendInput fallback
    ...
    # Interception scroll flag
    _IMS_WHEEL = 0x0400   # INTERCEPTION_MOUSE_WHEEL

    def _win_scroll(dy):
        if _USE_DRIVER and _icp_ctx is not None:
            stroke = _icp.InterceptionMouseStroke()
            stroke.state       = _IMS_WHEEL
            stroke.flags       = 0
            stroke.rolling     = int(dy * 120) & 0xFFFF   # signed 16-bit
            stroke.x           = 0
            stroke.y           = 0
            stroke.information = 0
            _icp.interception_send(_icp_ctx, _icp_mouse, stroke, 1)
    ...
    def _make_ki(vk, scan, flags):
        inp = INPUT(); inp.type = 1
        inp.ki.wVk = vk; inp.ki.wScan = scan
        inp.ki.dwFlags = flags; inp.ki.time = 0; inp.ki.dwExtraInfo = 0
        return inp

    def _send_ki(vk, scan, flags):
    ...
    def _win_press_mods(mods: int, down: bool):
        ...
        if _USE_DRIVER and _icp_ctx is not None:
            iflags = _IKOF_KEY_UP if not down else _IKOF_KEY_DOWN
            for bit, vk in _MOD_VK_PAIRS:
                if mods & bit:
                    scan   = _user32.MapVirtualKeyW(vk, 0) & 0xFF
                    stroke = _icp.InterceptionKeyStroke()
                    stroke.code        = scan
                    stroke.state       = iflags
                    stroke.information = 0
                    _icp.interception_send(_icp_ctx, _icp_kbd, stroke, 1)
        else:
    ...
    def _win_key(k, pressed, mods: int = 0):
        """Inject a key event via Interception driver (or SendInput fallback).

        Interception keyboard strokes use hardware scan codes, not VK codes,
        which means they bypass UIPI entirely and work on the Secure Desktop.
        """
        ...
        if _USE_DRIVER and _icp_ctx is not None:
            # ── Interception path: use hardware scan codes ────────────────────
            istate = _IKOF_KEY_UP if not pressed else _IKOF_KEY_DOWN

            if k and len(k) == 1:
                # Printable character: resolve VK → scan code.
                # Always use VkKeyScanW so that unmodified keys (including
                # Space, which has mods=0) go through the VK path instead of
                # being silently zeroed and falling through to Unicode injection.
                vk   = ord(k.upper()) if (mods and 0x20 <= ord(k) <= 0x7E) else 0
                scan = 0
                if vk:
                    scan = _user32.MapVirtualKeyW(vk, 0) & 0xFF
                if not scan:
                    # Covers space and all unmodified printable chars
                    vks  = _user32.VkKeyScanW(ord(k))
                    vk   = vks & 0xFF
                    scan = _user32.MapVirtualKeyW(vk, 0) & 0xFF if vk else 0
                if scan:
                    stroke = _icp.InterceptionKeyStroke()
                    stroke.code        = scan
                    stroke.state       = istate
            ...
            # ── SendInput fallback ────────────────────────────────────────────
            if k and len(k) == 1:
                # Use the virtual key when available (covers Space and all
                # standard ASCII).  Fall back to Unicode injection only for
                # characters that have no VK mapping (e.g. exotic Unicode).
                vks = _user32.VkKeyScanW(ord(k))
                vk  = vks & 0xFF
                if vk and vk != 0xFF:
